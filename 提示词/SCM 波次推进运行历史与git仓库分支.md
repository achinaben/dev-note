...
| 步骤 | 做什么 |
|------|--------|
| 1 | 在 GitHub 从 `main` 创建 **一条长期分支**，例如 `cursor/scm-wave`（名称自定，全仓统一即可） |
...
**首次没有 `cursor/scm-wave` 时：** 在 GitHub 网页从 `main` 新建该分支，或本机 `git checkout -b cursor/scm-wave main && git push -u origin cursor/scm-wave`，再让 Automation 以该分支为 startingRef。
...
| A3 | 确认远程已有 `cursor/scm-wave`（可先空分支，只有与 main 同起点也行） |
...
| Branch / gitConfig: `main` | 多为从 `main` **拉出**；**不等于** push 目标 |
| **推荐** startingRef / 工作分支 | `cursor/scm-wave` + **workOnCurrentBranch: true** |
