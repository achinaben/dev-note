...
- [ ] **Branch**：**`cursor/scm-wave`**（远程已建；见分支专题笔记 **2.7 节**）。
- [ ] **Work on current branch**：**开启**（若无此项见 2.7 节验收）。
...
## 防重 / 空转
4. 读 scm-platform/progress.md 与 scm-platform/AGENTS.md（先 cd scm-platform 再读）。
...
9. 更新 scm-platform/progress.md 运行日志；commit 后 push（见 Git 节）。

## Git（固定工作分支，禁止一 Run 一分支）
- 工作分支：cursor/scm-wave（全波次共用；已在该分支则不要 checkout -b 新分支）。
- 每轮成功：git add → commit → git push origin cursor/scm-wave；推送前可 git pull --rebase origin cursor/scm-wave。
- 禁止创建 cursor/agent-* 或任何新的随机 agent 分支；禁止 push 到 main；禁止自行 merge 到 main。
...
| 仍出现 `cursor/agent-*` | Work on current branch 未开或 Branch 仍为 main | 见第 7 节与分支专题 2.7 节 |
...
若仍出现 **`cursor/agent-xxxx`**：说明 **Work on current branch** 未开或 Branch 仍为 `main`。按分支专题笔记 **2.7** 改仪表盘；不要用「挑 agent 分支合并」当日常流程。
