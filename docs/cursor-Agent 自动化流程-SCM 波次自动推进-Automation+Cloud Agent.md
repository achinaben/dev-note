...
| GitHub | 默认在 `cursor/agent-*` 分支 push；**不会自动 merge 到 main**；合并后本机 `git pull` |
...
| 环节 | 行为 |
|------|------|
| Run 成功且有 push | 多在 **新的 `cursor/agent-xxxx`** |
| 合并 | **不会**自动进 `main` |
| 收口 | 专题笔记 **第 6.4 节** 批量处理历史分支，并尽快改为 7.1 |

更完整说明见：**SCM 波次推进：运行历史与 Git 仓库分支**（提示词目录，第 2 节起）。
